...
public class TreeEditDistance {
	 ...
	 * @param matchCost
	 */
	public TreeEditDistance(double delCost, double insCost, double matchCost) {
    ...
    /**
     * Cimputes the tree edit index between trees t1 and t2
     *
     * @param t1
     * @param t2
     * @return tree edit distance between trees t1 and t2
     */
    public double index(LabelTreeNode t1, LabelTreeNode t2) {
        int size = t1.getNodeCount() > t2.getNodeCount() ? t1.getNodeCount() : t2.getNodeCount();
        double distance = distance(t1, t2);

        return size > 0 ? distance / size : 0.0;
    }
	 ...
	 * @return tree edit distance between trees t1 and t2
	 */
	public double distance(LabelTreeNode t1, LabelTreeNode t2) {
		init(t1, t2);
	 ...
	 * @param t2
	 */
	public void init(LabelTreeNode t1, LabelTreeNode t2) {
	...
	private void forestDist(InfoTree treeInfo1, InfoTree treeInfo2, int i, int j, double[][] treedist, double[][] forestdist) {
		forestdist[treeInfo1.getInfo(POST2_LLD, i-1)+1 - 1][treeInfo2.getInfo(POST2_LLD, j-1)+1 - 1] = 0;
		for (int di = treeInfo1.getInfo(POST2_LLD, i-1)+1; di <= i; di++) {
			forestdist[di][treeInfo2.getInfo(POST2_LLD, j-1)+1 - 1] = forestdist[di - 1][treeInfo2.getInfo(POST2_LLD, j-1)+1 - 1] + costDel;
			for (int dj = treeInfo2.getInfo(POST2_LLD, j-1)+1; dj <= j; dj++) {
				forestdist[treeInfo1.getInfo(POST2_LLD, i-1)+1 - 1][dj] = forestdist[treeInfo1.getInfo(POST2_LLD, i-1)+1 - 1][dj - 1]	+ costIns;

				if ((treeInfo1.getInfo(POST2_LLD, di-1) == treeInfo1.getInfo(POST2_LLD, i-1))
						&& (treeInfo2.getInfo(POST2_LLD, dj-1) == treeInfo2.getInfo(POST2_LLD, j-1))) {
					double costRen = 0;
					if (!(treeInfo1.getInfo(POST2_LABEL, di-1) == treeInfo2.getInfo(POST2_LABEL, dj-1))) {
						costRen = costMatch;
					}
					...
					forestdist[di][dj] = Math.min(Math.min(
							forestdist[di - 1][dj] + costDel,
							forestdist[di][dj - 1] + costIns),
							forestdist[treeInfo1.getInfo(POST2_LLD, di-1)+1 - 1][treeInfo2.getInfo(POST2_LLD, dj-1)+1 - 1]
									+ treedist[di][dj]);
				}
			}
		}
